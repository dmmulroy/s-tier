...
let _ = Js.log (Sexp.from_string {| ( hello (who? world that's who)) |})
...
let main () =
   ...
   * placeholders inside the curly braces (e.g., `sexpression_test`) will be replaced 
   ...
   *)
  let configuration =
    Create_melange_app.Configuration.make ~name:"sexpression_test"
      ~directory:"/Users/ostera/repos/github.com/leostera/sexpression_test"
      ~node_package_manager:Npm ~bundler:Vite ~is_react_app:false
      ~initialize_git:true ~initialize_npm:true ~initialize_ocaml_toolchain:true
      ()
  in
