...
module Make (Sexp : Sexp) = struct
  ...
  module Parser = struct
    ...
    module Lexer = struct
      type state = Init | Parsing_length
      ...
      let feed t c =
        ...
        | Parsing_length, _ ->
            parse_errorf
              "invalid character %C while parsing atom length, expected \
               '0'..'9' or ':'"
              c
      ;;
  ...
  module Make_parser (Input : Input) = struct
    ...
    let ( >>=* ) m f =
      m >>= function Error _ as err -> return err | Ok x -> f x
    ;;
    ...
    let parse_many =
      let rec loop input lexer stack =
        ...
        | Ok c ->
            one_token input c lexer stack >>=* fun stack ->
            loop input lexer stack
      in
      fun input -> loop input (Lexer.create ()) Empty
    ;;
  end
